				...
				if ( ( UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )//非标准请求
				{
					...
					else
					{
						...
						if(soft_dtr == 1 && soft_rts == 1)
								{
									TXD1 = 1;
									CAP1_ = 1; // Usar P1.4 como alternativa a P1.0
								}
...
void SerialPort_Config()
{
	...
	TCLK = 0;																  //UART0发送时钟
	PCON |= SMOD;
	x = 10 * F_CPU / 230400 / 16;									   //如果更改主频，注意x的值不要溢出
	x2 = x % 10;
	x /= 10;
	if ( x2 >= 5 ) x ++;													   //四舍五入
...
void main(void)
{
	...
	uint16_t Esp_Stage = 0;	CfgFsys();														   //CH559时钟选择配置
	mDelaymS(5);														  //修改主频等待内部晶振稳定,必加
	
	// Habilitar CAP1 en pin alternativo P1.4 en lugar de P1.0
	PIN_FUNC |= bT2_PIN_X;  // Mapear CAP1 de P1.0 a P1.4
